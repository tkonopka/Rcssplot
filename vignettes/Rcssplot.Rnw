...
New functions for fetching values from Rcss objects: {\tt RcssValue}. The new functions are meant to reduce verbosity, especially when fetching values from a default style. 
