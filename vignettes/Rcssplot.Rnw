...
Note that the overloading operation masks several commands from base graphics. In principle, this masking could be defined within the package itself, and thus take effect automatically when the package is loaded. Future versions of the package may take this approach. Here, this mechanism is put in place to provide the user a decision when to activate the masking. Furthermore, the masking is active only within a local environment.
...
Other repetitive elements are the arguments {\tt Rcss=Rcss} and {\tt Rcssclass=Rcssclasss}. These arguments ensure that the style object and the class specified through the function call are passed on to the individual wrappers. We can avoid this repetition by setting a default style and a compulsory class through objects {\tt RcssDefaultStyle} and {\tt RcssCompulsoryClass}. 
...
Note that definitions of {\tt RcssDefaultStyle} and {\tt RcssCompulsoryClass} are achieved using function calls. Those functions are helpers that help preserve current default in certain cases. This will become more clear in the next section. 
...
\noindent The first result is {\tt foo0}; let's think of this as a css class that we wish to employ at a global level. In the first function {\tt foo1}, the compulsory class is set with a naive assignment. The return value reveals that within that function, the compulsory class becomes {\tt foo1} and the label {\tt foo0} is lost. The second function uses the {\tt RcssGetCompulsoryClass} function. This function looks up the compulsory class set in parent environments and augments it with the new label. This better reflects our intended behavior - i.e. to use the class {\tt foo0} in all cases. The final command displays that {\tt RcssCompulsoryClass} in the global environment is unaffected by the previous assignments. The use of the labels {\tt foo1} and {\tt foo2} are thus localized to the custom functions.

The function {\tt RcssGetDefaultStyle} fulfills an analogous role to {\tt RcssGetCompulsoryClass}. 
