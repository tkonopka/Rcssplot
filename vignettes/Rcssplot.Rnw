...
\begin{itemize}
  \item All Rcss wrappers change name to match the function names from base graphics. They automatically mask base graphics functions.
  \item RcssWatch utility
  ...
  \item Some function from 0.x versions are made redundant or deprecated. These provide warning messages, but will be removed in a future release.
\end{itemize}
