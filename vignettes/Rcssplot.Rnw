...
\noindent This command triggers some messages from the R environment. They convey that the package provides new implementations for functions that are already provided by base graphics. This means that executing commands with one of the listed names, for example {\tt mtext}, will launch implementations provided by Rcssplot rather than by the core package {\tt graphics}.
...
Next, let's use the style object in a plot. The Rcssplot package provides functions that mask and replicate many of R's graphics functions, but also add some new features. In practice, this means that familiar graphics commands can be used as before, or with additional arguments. For example, we can create a simple barplot (Figure \ref{fig:rcss_barplot}A).
...
While the code in {\tt rcss.barplot.1} is simpler than in {\tt base.barplot.1}, it still contains repetitive elements. In particualr, constructions {\tt Rcss=Rcss} and {\tt Rcssclass=Rcssclasss} appear in almost every line. We can avoid this repetition by setting a default style and a compulsory class. 

Handling of default is achieved through {\tt RcssDefaultStyle} and {\tt RcssCompulsoryClass}. These objects can be defined in any environment, for example inside a function. When present, the package functions use information therein to influence plot behavior. Consider the following adjustment of our barplot function.
...
In the previous example, {\tt rcss.barplot.2}, we changed the default style within the custom functions, i.e. in a local environment. In some cases, it may be reasonable to apply such changes in the global environment instead. 
...
\noindent When the styling is set as above, in the global environment, the custom barplot function can be simplified even further.
...
We've already seen that Rcss files can store settings for familiar graphics settings. But these cascading style sheets can also be used to encode other settings as well, indeed any property/value pairs. Consider style file {\tt vignettes.bar4.Rcss}.

<<barrcss_4, eval=TRUE, echo = FALSE, comment=NA>>=
showrcss("Rcss/vignettes.bar4.Rcss")
@

\noindent The first block is named {\tt baraxis}, but this does not correspond to any of R's base graphics commands. Therefore, this block does not affect any Rcssplot functions. But we can write code to exploit information in {\tt baraxis} by extracting values manually. The package provides two functions for this purpose, {\tt RcssProperty} and {\tt RcssValue}.
...
\noindent In this example, the utility loads styles provided in files {\tt style.files} and evaluates {\tt rcss.barplot.4} with argument {\tt x=a}. It then waits for a keystroke before attempting the same over and over again. With this tool, adjustments in style definitions can be edited in Rcss files with text editor, and the effects previewed with one keystroke. The utility also accepts files with extension {\tt .R} or {\tt .r}, so it helps preview changes to the definition of the custom plot function.
...
\begin{itemize}
  \item All Rcss wrappers change name to match the function names from base graphics. They automatically overload base graphics functions. 
...
\begin{itemize}
 \item New functions for getting and setting values from Rcss objects: {\tt RcssValue}, {\tt RcssUpdate}. These functions are complementary to previously exisitng functions, but are less verbose, epsecially forfetching values from a default style. 
